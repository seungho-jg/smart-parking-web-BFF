...
public class SessionManager implements SessionManagerInterface {
    ...
    @Override
    public synchronized String create(String id, String name) {
        // 세션 아이디 생성시간 + hash값
       String sessionId = Instant.now().toString() + System.identityHashCode(name);
       sessions.put(sessionId, new SessionInfo(id, name));
       return sessionId;
    }
